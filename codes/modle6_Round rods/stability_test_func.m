...
global T_diag V2 A ne w L d_min1 d_min3  
%% 考虑外力F=10000
...
for i=1:ne
    ...
    d_tuple1{i,:}=max(min(d1_p),min(d2_p));    % d_tuple： 各个杆件满足条件截面积最小值组成的元组 
end
d_min1=cell2mat(d_tuple1);   % d_min2：行：表示各个杆件截面积最小值
%% 稳定性验算
d_tuple2=root(d_min1);   % d3_tuple：各个杆件满足条件截面积最小值组成的元组 
d_min2=cell2mat(d_tuple2);   % d_min2：行：表示各个杆件截面积最小值
xy=[d_min1,d_min2]';d1=max(xy);
d_min3=0.25*pi*d1.^2*1e6;   % d_min3：杆件截面积最小值(mm2)
